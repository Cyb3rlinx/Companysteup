...
try{
 ...
 assert(!created.error&&created.data,'Synthetic agent conversation fixture failed');
  ...
  assert(!ownConversation.error&&ownConversation.data.length===1&&!ownDelaware.error&&ownDelaware.data.length===1&&!ownEstonia.error&&ownEstonia.data.length===1&&!ownTurn.error&&ownTurn.data.length===1,'Owner cannot read conversation fixture');
  ...
  assert(!foreignConversations.error&&foreignConversations.data.length===0&&!foreignDelaware.error&&foreignDelaware.data.length===0&&!foreignEstonia.error&&foreignEstonia.data.length===0&&!foreignTurns.error&&foreignTurns.data.length===0,'Conversation leaked to another authenticated tenant');
