...
try{
 ...
 assert(!created.error&&created.data,'Synthetic agent conversation fixture failed');
  ...
  const estonia=await admin.from('agent_conversations').insert({organization_id:a.org,case_id:a.cases.EE,jurisdiction_code:'EE',status:'active',execution_mode:'DETERMINISTIC_MOCK',synthetic:true,state_json:{},pending_patch:{},revision:0,client_request_id:randomUUID(),created_by:a.identity.id}).select('id,jurisdiction_code').single();
  assert(!estonia.error&&estonia.data?.jurisdiction_code==='EE','Estonia agent conversation migration not active');
  const uk=await admin.from('agent_conversations').insert({organization_id:a.org,case_id:a.cases.GB,jurisdiction_code:'GB',status:'active',execution_mode:'DETERMINISTIC_MOCK',synthetic:true,state_json:{},pending_patch:{},revision:0,client_request_id:randomUUID(),created_by:a.identity.id}).select('id,jurisdiction_code').single();
  assert(!uk.error&&uk.data?.jurisdiction_code==='GB','UK agent conversation migration not active');
  ...
  const ownEstonia=await a.client.from('agent_conversations').select('id').eq('id',estonia.data.id);
  const ownUk=await a.client.from('agent_conversations').select('id').eq('id',uk.data.id);
  const ownTurn=await a.client.from('agent_conversation_turns').select('id').eq('id',turn.data.id);
  assert(!ownConversation.error&&ownConversation.data.length===1&&!ownDelaware.error&&ownDelaware.data.length===1&&!ownEstonia.error&&ownEstonia.data.length===1&&!ownUk.error&&ownUk.data.length===1&&!ownTurn.error&&ownTurn.data.length===1,'Owner cannot read conversation fixture');
  ...
  const foreignEstonia=await b.client.from('agent_conversations').select('id').eq('id',estonia.data.id);
  const foreignUk=await b.client.from('agent_conversations').select('id').eq('id',uk.data.id);
  const foreignTurns=await b.client.from('agent_conversation_turns').select('id').eq('id',turn.data.id);
  assert(!foreignConversations.error&&foreignConversations.data.length===0&&!foreignDelaware.error&&foreignDelaware.data.length===0&&!foreignEstonia.error&&foreignEstonia.data.length===0&&!foreignUk.error&&foreignUk.data.length===0&&!foreignTurns.error&&foreignTurns.data.length===0,'Conversation leaked to another authenticated tenant');
