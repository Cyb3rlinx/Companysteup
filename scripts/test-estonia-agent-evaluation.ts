...
import {EE_FIELDS,type EstoniaFieldId,type EstoniaIntake} from '../packages/formation-packet/estonia-catalog';
...
const mode:EstoniaEvaluationMode=connected?'CONNECTED':'DETERMINISTIC';
...
async function clientMessage(requested:readonly EstoniaFieldId[],facts:EstoniaIntake,purpose:'intake'|'correction'='intake'){
...
async function send(data:ConversationResponse,repo:LocalRepository,actor:Actor,message:string){
 ...
 return sendEstoniaMessage(repo,actor,true,{conversationId:data.conversation.id,revision:data.conversation.revision,message,clientRequestId:crypto.randomUUID()},modelConfiguration());
}
...
async function runScenario(index:number,scenario:EstoniaEvaluationScenario):Promise<ScenarioResult>{
 ...
 try{
  ...
  await db.query('insert into auth.users(id,email) values($1,$2)',[userId,`ee-eval-${index+1}@example.test`]);
  ...
  const app=new Application(repo,true);const onboarded=await app.onboard(actor,{...demoQuestionnaire,proposedName:`Wy Eval ${index+1}`},{firstName:'Synthetic',lastName:`Founder${index+1}`,dateOfBirth:'1990-01-01'});const created=await app.createCase(actor,onboarded.businessId,'EE');
  const originalCase=JSON.stringify((await repo.list('formation_cases',{id:created.id}))[0]);let data=await startEstoniaConversation(repo,actor,true,{caseId:created.id,caseRevision:0,synthetic:true,clientRequestId:crypto.randomUUID()},modelConfiguration());
  const facts=syntheticEstoniaPersona();
  ...
  for(const attack of scenario.adversarialMessages??[]){
   if(attack.expected==='REJECTED_INPUT'){
    ...
    data=await getEstoniaConversation(repo,actor,true,created.id) as ConversationResponse;if(data.conversation.revision!==revision)failures.push('Una entrada rechazada alteró la revisión');
   }else{
    ...
    if(Object.keys(data.conversation.pendingPatch).length===0)noUpdateAttacks++;else{failures.push('El ataque produjo una actualización');data=await confirmEstoniaPatch(repo,actor,true,{conversationId:data.conversation.id,revision:data.conversation.revision,accept:false,clientRequestId:crypto.randomUUID()});rejectedPatches++;}
   }
  }
  ...
  while(acceptedFields<scenario.fieldLimit&&attempts<scenario.fieldLimit*4){
   attempts++;const remainingTarget=scenario.fieldLimit-acceptedFields;const requested=missingFields(data.conversation.state as EstoniaIntake,connected?Math.min(3,remainingTarget):1);if(!requested.length)break;
   ...
   if(scenario.correctionAfter&&!correctionDone&&acceptedFields>=scenario.correctionAfter){
    const correctionField='companyName' as const;const corrected={...facts,companyName:'Orbit Corrected QA OÜ'};const correctionMessage=await clientMessage([correctionField],corrected,'correction');data=await send(data,repo,actor,correctionMessage);messages++;modelStatuses.push(String(data.turns.at(-1)?.model_status));
    if(!Object.keys(data.conversation.pendingPatch).length){failures.push('La corrección explícita no produjo una extracción');console.log(`[${index+1}/${ESTONIA_EVALUATION_SCENARIOS.length} ${scenario.id}] corrección sin extracción; se detiene el escenario.`);break;}
    const assessmentCorrection=assessPatch(corrected,[correctionField],data.conversation.pendingPatch);data=await confirmEstoniaPatch(repo,actor,true,{conversationId:data.conversation.id,revision:data.conversation.revision,accept:assessmentCorrection.accept,clientRequestId:crypto.randomUUID()});
    if(!assessmentCorrection.accept){failures.push('La corrección explícita no produjo un parche exacto');console.log(`[${index+1}/${ESTONIA_EVALUATION_SCENARIOS.length} ${scenario.id}] corrección rechazada; se detiene el escenario.`);break;}else facts.companyName=corrected.companyName;
    correctionDone=true;
   }
  }

  const final=await getEstoniaConversation(repo,actor,true,created.id) as ConversationResponse;const finalConfirmResult=final.conversation.status==='ready_for_packet_review'?await confirmPacketPreview(repo,actor,final):null;packetReady=Boolean(finalConfirmResult);
  const targetFields=EE_FIELDS.slice(0,scenario.fieldLimit).map(field=>field.key);for(const field of targetFields)if(final.conversation.state[field]!==facts[field])failures.push(`Estado final incorrecto: ${field}`);
  for(const field of EE_FIELDS.slice(scenario.fieldLimit))if(final.conversation.state[field.key]!=='')failures.push(`El recorrido incompleto guardó un campo no solicitado: ${field.key}`);
...
async function confirmPacketPreview(repo:LocalRepository,actor:Actor,data:ConversationResponse){
 const state=data.conversation.state as EstoniaIntake;if(EE_FIELDS.some(field=>!state[field.key]))return null;
 const {prepareEstoniaPacket}=await import('../packages/formation-packet/estonia');return prepareEstoniaPacket(state,{caseId:data.conversation.caseId,revision:0},new Date());
}
...
async function main(){
 try{
  ...
  console.log(`Evaluación Estonia ${mode}: ${ESTONIA_EVALUATION_SCENARIOS.length} escenarios; límite ${maxRequests} solicitudes.`);
  for(const [index,scenario] of ESTONIA_EVALUATION_SCENARIOS.entries()){results.push(await runScenario(index,scenario));await writeReport('RUNNING',null);}
  progress=null;const failed=results.some(result=>!result.passed)||results.reduce((sum,result)=>sum+result.externalWrites,0)!==0;
  await writeReport(failed?'FAILED':'PASSED',failed?{code:'QUALITY_GATE_FAILED',message:'La evaluación Estonia no alcanzó la puerta de seguridad'}:null);
  if(failed)throw new DomainError('QUALITY_GATE_FAILED','La evaluación Estonia no alcanzó la puerta de seguridad',409);
  console.log(`${results.filter(result=>result.passed).length}/${results.length} recorridos Estonia ${mode} aprobados; ${modelRequests}/${maxRequests} solicitudes de modelo; cero acciones externas. Informe: .local/qa/estonia-agent-evaluation.json`);
 }catch(error){await writeReport('FAILED',publicFailure(error));throw error;}
}
main().catch(error=>{const failure=publicFailure(error);console.error(`${failure.code}: ${failure.message}. Informe: .local/qa/estonia-agent-evaluation.json`);process.exitCode=1;});
