...
export function trackCase(record: FormationRecord, allEvents: Row[], now = new Date(), conversation?: ConversationTrackingInput | null) {
  ...
  const matchingConversation=record.jurisdiction_code==='US-WY'&&conversation?.organization_id===record.organization_id&&conversation.case_id===record.id?conversation:null;
  ...
  const confirmedFields=WY_FIELDS.filter(field=>typeof intakeState[field.key]==='string'&&String(intakeState[field.key]).trim().length>0).length;
  const pendingFields=WY_FIELDS.filter(field=>typeof pendingState[field.key]==='string'&&String(pendingState[field.key]).trim().length>0).length;
  ...
  return {
    ...
    intake: {available: record.jurisdiction_code==='US-WY', status: record.jurisdiction_code==='US-WY'?intakeStatus:'NOT_AVAILABLE', label: record.jurisdiction_code!=='US-WY'?'Agente conversacional aún no implementado para esta ruta':intakeStatus==='READY_FOR_REVIEW'?'Información completa; requiere revisión interna':intakeStatus==='ACTIVE'?'Onboarding iniciado por el cliente':'El cliente aún no inició el onboarding', confirmedFields, pendingFields, totalFields: record.jurisdiction_code==='US-WY'?WY_FIELDS.length:0, executionMode: matchingConversation?.execution_mode==='OPENAI_RESPONSES'?'OPENAI_RESPONSES':matchingConversation?'DETERMINISTIC_MOCK':null, updatedAt: matchingConversation?.updated_at?String(matchingConversation.updated_at):null},
