...
export type ListOptions={orderBy:string;descending?:boolean;limit?:number};
function listOptions(options?:ListOptions){if(options&&(!Number.isInteger(options.limit??1000)||(options.limit??1000)<1||(options.limit??1000)>1000))throw new DomainError('INVALID_LIMIT','Límite de consulta inválido');return options?.limit??1000;}
export interface Repository {
 list<T extends Row=Row>(table:string,where?:Row,options?:ListOptions):Promise<T[]>;
...
export class LocalRepository implements Repository {
 ...
 async list<T extends Row=Row>(table:string,where:Row={},options?:ListOptions):Promise<T[]> {
  tableName(table);const limit=listOptions(options);const keys=Object.keys(where);const query=`select * from public.${identifier(table)}${keys.length?' where '+keys.map((k,i)=>`${identifier(k)}=$${i+1}`).join(' and '):''}${options?' order by '+identifier(options.orderBy)+(options.descending?' desc':' asc'):''} limit ${limit}`;
...
export class SupabaseRepository implements Repository {
 ...
 async list<T extends Row=Row>(table:string,where:Row={},options?:ListOptions):Promise<T[]>{const limit=listOptions(options);let query=this.client.from(tableName(table)).select('*');for(const [k,v]of Object.entries(where))query=v===null?query.is(k,null):query.eq(k,v);if(options){identifier(options.orderBy);query=query.order(options.orderBy,{ascending:!options.descending});}const{data,error}=await query.limit(limit);if(error)throw new DomainError('DB_ERROR','No se pudo consultar el recurso',500);return data as T[];}
