...
export function fieldsForJurisdiction(jurisdiction:ConversationJurisdiction):readonly IntakeField[]{return jurisdiction==='US-DE'?DE_FIELDS:WY_FIELDS;}
export function intakeSchemaFor(jurisdiction:ConversationJurisdiction):z.ZodType<Record<string,string>>{return (jurisdiction==='US-DE'?delawareIntakeSchema:wyomingIntakeSchema) as z.ZodType<Record<string,string>>;}
export function initialConversationState(jurisdiction:ConversationJurisdiction='US-WY'):Record<string,string>{return jurisdiction==='US-DE'?emptyDelawareIntake():emptyWyomingIntake();}
...
export function safeSyntheticMessage(value:unknown){
 ...
 if(/\b(?:\d{3}-\d{2}-\d{4}|passport|pasaporte|api[_ -]?key|secret(?:o)?|contrase(?:ñ|n)a|fedex\s+account|ups\s+account)\b/i.test(message))throw new DomainError('SENSITIVE_DATA_REJECTED','No ingreses identificadores, pasaportes, cuentas de mensajería, credenciales ni secretos en el laboratorio.');
