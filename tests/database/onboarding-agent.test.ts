...
test('Wyoming conversation is private, idempotent, confirmed and cannot mutate the case',async()=>{
 const db=await testDatabase();try{
  ...
  let correctionRequest='';const modelCorrection=async(_url:string,init?:RequestInit)=>{correctionRequest=String(init?.body);return new Response(JSON.stringify({output:[{type:'function_call',name:'propose_wyoming_intake_update',arguments:JSON.stringify({updates:[{field:'companyName',value:'Orbit Model LLC',evidence:'nombre corregido'}]})}]}),{status:200,headers:{'Content-Type':'application/json'}});};
  const proposedByModel=await sendWyomingMessage(repo,actors[0],true,{conversationId:started.conversation.id,revision:6,message:'Corrijo Nombre propuesto: Orbit Model LLC',clientRequestId:'10000000-0000-4000-8000-000000000011'},{key:'test-key',model:'test-model',failureMode:'throw',fetcher:modelCorrection as typeof fetch});expect(proposedByModel.conversation.pendingPatch).toEqual({companyName:'Orbit Model LLC'});expect(correctionRequest).toContain('companyName');expect(correctionRequest).not.toContain('contactName');
  const confirmedByModel=await confirmWyomingPatch(repo,actors[0],true,{conversationId:started.conversation.id,revision:7,accept:true,clientRequestId:'10000000-0000-4000-8000-000000000012'});expect(confirmedByModel.conversation.state.companyName).toBe('Orbit Model LLC');
  const unavailable=async()=>new Response('',{status:404});
  await expect(sendWyomingMessage(repo,actors[0],true,{conversationId:started.conversation.id,revision:8,message:'Actividad ficticia: Software QA',clientRequestId:'10000000-0000-4000-8000-000000000010'},{key:'test-key',model:'missing-model',failureMode:'throw',fetcher:unavailable as typeof fetch})).rejects.toMatchObject({code:'MODEL_UNAVAILABLE',message:expect.stringContaining('HTTP 404')});
