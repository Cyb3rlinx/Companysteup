...
test('Wyoming conversation is private, idempotent, confirmed and cannot mutate the case',async()=>{
 const db=await testDatabase();try{
  ...
  const proposedByModel=await sendWyomingMessage(repo,actors[0],true,{conversationId:started.conversation.id,revision:6,message:'El nuevo nombre será Orbit Model LLC',clientRequestId:'10000000-0000-4000-8000-000000000011'},{key:'test-key',model:'test-model',failureMode:'throw',fetcher:modelCorrection as typeof fetch});expect(proposedByModel.conversation.pendingPatch).toEqual({companyName:'Orbit Model LLC'});expect(correctionRequest).toContain('companyName');
