...
test('connected synthetic client uses one strict tool, store false and rejects altered facts',async()=>{
 ...
 const correction=async()=>new Response(JSON.stringify({output:[{type:'function_call',name:'reply_as_synthetic_customer',arguments:JSON.stringify({message:`Corrijo los siguientes datos:\nNombre propuesto: ${facts.companyName}`,disclosedFields:['companyName']})}]}),{status:200,headers:{'Content-Type':'application/json'}});
 await expect(openAISyntheticClientMessage(['companyName'],facts,{key:'test-key',model:'test-model',purpose:'correction'},correction as typeof fetch)).resolves.toMatchObject({disclosedFields:['companyName']});
 ...
 const partial=async()=>new Response(JSON.stringify({output:[{type:'function_call',name:'reply_as_synthetic_customer',arguments:JSON.stringify({message:`Nombre propuesto: ${facts.companyName}`,disclosedFields:['companyName']})}]}),{status:200,headers:{'Content-Type':'application/json'}});
