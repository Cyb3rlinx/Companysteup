...
test('OpenAI adapter stores no response, forces one strict extraction tool and ignores free prose',async()=>{
 let request:Record<string,unknown>|undefined;let observed:Record<string,number>|undefined;let validation:Record<string,unknown>|undefined;
 ...
 const result=await extractWithOpenAI('Mi nombre es Orbit QA LLC',['companyName'],{key:'test-key',model:'test-model',onMetrics:metrics=>{observed=metrics;},onValidation:value=>{validation=value;}},fetcher as typeof fetch);
 ...
 expect(JSON.stringify(request)).not.toContain('test-key');expect(JSON.stringify(request)).not.toContain('agentState');expect(observed).toMatchObject({inputTokens:10,outputTokens:5,totalTokens:15});expect(validation).toMatchObject({proposedUpdates:1,acceptedUpdates:1,rejections:[]});
