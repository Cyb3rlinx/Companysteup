...
La evaluación de Wyoming tiene dos niveles. `pnpm test:wyoming-agent` ejecuta cuatro recorridos deterministas sin red. Para `pnpm test:wyoming-agent:connected`, configurar además `OPENAI_SIMULATOR_MODEL` y opcionalmente `OPENAI_EVAL_MAX_REQUESTS` entre 1 y 200; el valor por defecto es 60. El simulador recibe solo datos ficticios y debe devolver exactamente todos los campos solicitados. El onboarding permite una corrección explícita de un campo confirmado, pero no puede persistir una paráfrasis: el texto libre debe existir literalmente en el mensaje y se usa como evidencia; las opciones cerradas usan valores canónicos con evidencia literal. El modo conectado muestra progreso, falla de inmediato ante errores OpenAI o una extracción sin avance y guarda un reporte saneado incluso al abortar en `.local/qa/wyoming-agent-evaluation.json`, incluidos contadores de validación sin valores. No aumentar el presupuesto antes de revisar `failure` y `progress`. No colocar claves en argumentos, reportes, Git ni conversaciones. Ver `WYOMING_AGENT_EVALUATION.md`.
