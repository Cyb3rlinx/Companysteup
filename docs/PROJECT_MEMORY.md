...
- Las conversaciones Wyoming, Delaware y Estonia están integradas en el detalle del expediente. El cliente controla sus respuestas y confirmaciones; operaciones tiene lectura. El seguimiento solo devuelve jurisdicción, estado, conteos y eventos saneados, sin valores del intake. Fuente: `apps/web/components/wyoming-conversation.tsx`, `packages/case-tracking/index.ts` y `packages/onboarding-agent/service.ts`.
...
- Supabase migrations es la autoridad del esquema; PGlite local usa las mismas migraciones. El staging autorizado tiene trece migraciones, 57 tablas con RLS y diez Edge Functions. La migración 013 permite conversaciones `US-WY | US-DE | EE` y conserva RLS. Fuente: `docs/SESSION_HANDOFF.md` y `docs/BUILD_STATUS.md`.
...
- Wyoming y Delaware aprobaron 4/4 recorridos deterministas y conectados. Estonia tiene 24 campos propios, paquete, conversación, panel, migración/RLS y 4/4 determinista. El primer run conectado falló cerrado 2/4 por rechazar parches exactos parciales; `2026-09-15.1` conserva los omitidos pendientes y solo confirma progreso literal sin valores incorrectos o inesperados. Siguiente paso: repetir la puerta conectada Estonia; después UK. Fuente: `docs/WYOMING_AGENT_EVALUATION.md`, `docs/DELAWARE_AGENT_EVALUATION.md`, `docs/ESTONIA_AGENT_EVALUATION.md` y `docs/SESSION_HANDOFF.md`.
