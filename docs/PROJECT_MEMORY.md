...
- Los agentes preparan expedientes y orientan con controles deterministas; todavía no constituyen empresas reales de forma autónoma. Wyoming, Delaware, Estonia y UK aprobaron evaluaciones conectadas sobre datos ficticios y cero acciones externas. La evidencia Wyoming es anterior a la versión vigente de su evaluador y requiere repetición; no hay pagos reales ni presentación gubernamental habilitada. Fuente: `docs/WYOMING_AGENT_EVALUATION.md`, `docs/DELAWARE_AGENT_EVALUATION.md`, `docs/ESTONIA_AGENT_EVALUATION.md`, `docs/UK_AGENT_EVALUATION.md` y `docs/BUILD_STATUS.md`.
...
- Las conversaciones Wyoming, Delaware, Estonia y UK están integradas en el detalle del expediente. El cliente controla sus respuestas y confirmaciones; operaciones tiene lectura. El seguimiento solo devuelve jurisdicción, estado, conteos y eventos saneados, sin valores del intake. Fuente: `apps/web/components/wyoming-conversation.tsx`, `packages/case-tracking/index.ts` y `packages/onboarding-agent/service.ts`.
- `packages/agent-operations` compara cada evidencia conectada con la versión actual del evaluador. El admin expone este estado y una cola por caso con próximo campo saneado, excepciones, responsable, evento y SLA interno; esta información operativa no se entrega al cliente. Fuente: `packages/agent-operations/index.ts`, `packages/case-tracking/index.ts` y `apps/web/components/workspace.tsx`.
...
- Supabase migrations es la autoridad del esquema; PGlite local usa las mismas migraciones. El staging autorizado tiene catorce migraciones, 57 tablas con RLS y diez Edge Functions. La migración 014 permite conversaciones `US-WY | US-DE | EE | GB` y conserva RLS. Fuente: `docs/SESSION_HANDOFF.md` y `docs/BUILD_STATUS.md`.
...
- Las cuatro rutas aprobaron 4/4 recorridos deterministas y conectados en sus versiones vigentes, con cero acciones externas. Wyoming `2026-09-14.5` cerró la última diferencia de versión el 2026-09-15. Fuente: `docs/WYOMING_AGENT_EVALUATION.md`, `docs/DELAWARE_AGENT_EVALUATION.md`, `docs/ESTONIA_AGENT_EVALUATION.md`, `docs/UK_AGENT_EVALUATION.md` y `docs/SESSION_HANDOFF.md`.
...
- Las credenciales del modelo permanecen exclusivamente en la terminal privada y no son necesarias para repetir el nivel determinista. Siguiente puerta: Google OAuth alojado; el staging confirma que el proveedor continúa deshabilitado. Después corresponde hosting y piloto supervisado. Para lanzamiento hay que identificar operador y responsable interno, y resolver servicios externos solo cuando la ruta o actividad los requiera. Fuente: `docs/SESSION_HANDOFF.md` y `docs/GOOGLE_AUTH.md`.
