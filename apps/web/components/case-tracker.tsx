...
const jurisdictionLabels:Record<string,string>={'US-WY':'Wyoming','US-DE':'Delaware',EE:'Estonia',GB:'UK'};
export function CaseTracker({caseId, names = {}, internal = false, onUpdate}: {caseId?: string; names?: Record<string,string>; internal?: boolean; onUpdate?:()=>Promise<void>}) {
  ...
  return <section className={styles.tracker} aria-label={internal?'Seguimiento operativo de expedientes':'Seguimiento de mis expedientes'}>
    ...
    <div className={styles.grid}>{cases.map(item=><article key={item.caseId} className={styles.card}>
      ...
      <dl><dt>Paso actual</dt><dd>{item.status}</dd><dt>Responsable del paso</dt><dd>{item.nextOwner}</dd><dt>Asistente de la ruta</dt><dd>{item.agent.name}<small>Preparación determinista · v{item.agent.version}</small></dd><dt>Última ejecución</dt><dd>{busy===item.caseId?'Preparando el resumen solicitado…':runLabels[item.agent.runStatus]}</dd>{item.intake.available&&<><dt>Onboarding {jurisdictionLabels[String(item.intake.jurisdiction)]}</dt><dd>{item.intake.label}<small>{item.intake.confirmedFields}/{item.intake.totalFields} campos confirmados{item.intake.pendingFields?` · ${item.intake.pendingFields} pendientes de confirmación`:''}</small></dd></>}</dl>
      {internal&&<div className={styles.operations}><b>{item.operations.attentionLabel}</b><dl><dt>Responsable</dt><dd>{item.operations.owner}</dd><dt>Dato siguiente</dt><dd>{item.operations.nextMissingField??'Intake completo'}</dd><dt>Excepciones</dt><dd>{item.operations.openEscalations}{item.operations.highestSeverity?` · prioridad ${item.operations.highestSeverity}`:''}</dd><dt>Último evento</dt><dd>{item.operations.latestEvent?`${item.operations.latestEvent.label} · ${new Date(item.operations.latestEvent.at).toLocaleString('es-419')}`:'Sin actividad registrada'}</dd><dt>SLA interno</dt><dd>{item.operations.slaDueAt?`${item.operations.slaBreached?'Vencido':'Vence'} ${new Date(item.operations.slaDueAt).toLocaleString('es-419')}`:'No corre mientras esperamos al cliente'}<small>Política {item.operations.policyVersion}; no es un plazo legal.</small></dd></dl></div>}
