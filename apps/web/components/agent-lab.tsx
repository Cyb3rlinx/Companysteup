...
import {DelawareConversationLab,EstoniaConversationLab,UkConversationLab,WyomingConversationLab} from './wyoming-conversation';
...
export function AgentLab({name, version, observedAt, recheckAfter, guides, cases, readOnly=false}: {
  name: string; version: string; observedAt: string; recheckAfter: string;
  guides: Guide[]; cases: {id: string; jurisdiction: string; revision:number}[]; readOnly?:boolean;
}) {
  ...
  return <main className={styles.lab}>
    ...
    <div className={styles.heading}><div><p className={styles.eyebrow}>CONTROL DE ALCANCE · {version}</p><h1>Laboratorio de agentes</h1><p>Un recorrido por jurisdicción. Cada dato con su destino; cada límite con su responsable.</p></div><button className="btn" disabled={busy} onClick={() => run(true)}>{busy ? 'Evaluando…' : `Evaluar las ${guides.length} rutas`}</button></div>
    ...
    {selected==='EE'&&<div className={styles.content}><EstoniaConversationLab cases={cases.filter(c=>c.jurisdiction==='EE')} readOnly={readOnly}/></div>}
    {selected==='GB'&&<div className={styles.content}><UkConversationLab cases={cases.filter(c=>c.jurisdiction==='GB')} readOnly={readOnly}/></div>}
